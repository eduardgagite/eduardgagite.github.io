          ...
          <div className="relative w-full max-w-[min(360px,90vw)]">
            ...
            <div className="relative rounded-[28px] border border-white/10 bg-white/[0.04] backdrop-blur-xl p-5 overflow-hidden shadow-[0_28px_70px_-40px_rgba(0,0,0,0.85)]">
              ...
              <div className="mt-5">
                ...
                <p className="mt-1.5 text-sm font-medium text-theme-accent">{t('hero.role')}</p>
              </div>
...
function HeroCta({ to, label, primary }: { to: string; label: string; primary?: boolean }) {
  return (
    <Link
      to={to}
      className={`inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg border px-4 py-2.5 text-xs font-medium transition-all ${
        primary
          ? 'border-transparent text-white hover:brightness-110'
          : 'border-white/10 bg-white/[0.05] text-white/90 hover:border-white/20 hover:bg-white/[0.1]'
      }`}
      style={primary ? { background: 'rgba(var(--theme-primary-rgb), 0.35)' } : undefined}
    >
      {label}
...
function ContactLink({ href, icon, children, external }: ContactLinkProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0 flex flex-col text-sm">{children}</div>
      <svg
        className="w-4 h-4 text-white/30 group-hover/link:text-white/50 transition-all"
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
