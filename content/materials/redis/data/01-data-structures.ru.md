...
| Структура | Типичный сценарий |
| --- | --- |
| String | кэш, счётчик, токен, сериализованное значение |
| Hash | объект с независимо обновляемыми полями |
| List | очередь, стек, последовательность последних событий |
| Set | уникальные элементы и операции над множествами |
| Sorted Set | рейтинг, приоритетная очередь, диапазоны по score |
| Stream | журнал событий и группы потребителей |
