...
От обычных множеств они отличаются тем, что каждый элемент имеет числовой «вес», по которому Redis сортирует коллекцию. Этот тип хорошо подходит для рейтингов, лидбордов, статистики, где важно быстро узнать позиции элементов или взять часть списка по определённому диапазону.
...
Эта структура рассчитана на приближённый подсчёт количества уникальных значений. Она не хранит сами элементы, зато позволяет оценить, сколько уникальных записей прошло через систему. Подходит для огромных массивов данных, где точность важна меньше, чем компактность и скорость (например, количество уникальных посетителей).
