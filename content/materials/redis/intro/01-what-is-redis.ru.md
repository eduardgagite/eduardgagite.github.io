...
### Как правильно сравнивать производительность

Redis нельзя напрямую сравнивать с SQL-базой только по одной цифре latency: инструменты решают разные задачи. Redis оптимизирован для простых операций по ключу в памяти, а PostgreSQL и MySQL дают транзакции, связи, сложные запросы и надёжное дисковое хранение.

На практике сравнивают не абстрактные `INSERT` и `UPDATE`, а конкретный сценарий приложения:

| Что измерять | Почему это важно |
| --- | --- |
| p50, p95 и p99 latency | среднее значение скрывает редкие, но заметные задержки |
| throughput под целевой нагрузкой | показывает, сколько операций выдерживает система |
| объём и структура данных | влияют на память, сеть и время обработки |
| persistence и репликацию | надёжность всегда имеет свою стоимость |

Redis обычно выигрывает в простых операциях чтения и записи по ключу, но итог зависит от конфигурации, сети, persistence и модели данных. Поэтому решение стоит принимать по benchmark своего сценария, а не по универсальному графику.
