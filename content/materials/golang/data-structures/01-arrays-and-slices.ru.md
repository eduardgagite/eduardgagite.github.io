...
1. Напишите функцию `Filter(nums []int, predicate func(int) bool) []int`, которая возвращает новый срез с элементами, удовлетворяющими условию.
2. Напишите функцию `Unique(s []string) []string`, которая убирает дубликаты из среза (порядок не важен).
3. Реализуйте простой стек на базе среза: `Push(val int)`, `Pop() (int, bool)`, `Peek() (int, bool)`.
