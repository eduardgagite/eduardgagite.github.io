...
1. Напишите функцию `DaysUntil(date string) (int, error)`, которая принимает дату в формате `"2026-12-31"` и возвращает количество дней до этой даты.
2. Напишите функцию `MeasureExec(fn func()) time.Duration`, которая измеряет время выполнения переданной функции.
3. Напишите программу, которая каждые 2 секунды выводит текущее время в формате `15:04:05`, и останавливается через 10 секунд. Используйте `time.NewTicker` и `time.After`.
