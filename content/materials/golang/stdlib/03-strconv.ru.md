...
1. Напишите функцию `ParseCSVInts(csv string) ([]int, error)`, которая принимает строку `"1,2,3,42"` и возвращает `[]int{1, 2, 3, 42}`. Используйте `strings.Split` и `strconv.Atoi`.
2. Напишите конвертер систем счисления: функцию `ConvertBase(num string, fromBase, toBase int) (string, error)`, которая преобразует число из одной системы счисления в другую через `strconv.ParseInt` и `strconv.FormatInt`.
3. Напишите функцию `FormatFileSize(bytes int64) string`, которая форматирует размер файла в человекочитаемый вид: `1024` → `"1.0 KB"`, `1048576` → `"1.0 MB"`.
