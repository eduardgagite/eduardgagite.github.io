...
1. Создайте тип `Money` с полями `Amount float64` и `Currency string`. Реализуйте метод `String()`, чтобы `fmt.Println(Money{99.99, "USD"})` выводил `$99.99`.
2. Напишите функцию `FormatTable(headers []string, rows [][]string) string`, которая форматирует данные в текстовую таблицу с выровненными колонками через `fmt.Sprintf` и `%-*s`.
3. Напишите функцию, которая принимает `error` и оборачивает её через `fmt.Errorf` с `%w`, добавляя контекст. Проверьте, что `errors.Is` находит оригинальную ошибку в цепочке.
