...
1. Напишите функцию `TopN(nums []int, n int) []int`, которая возвращает N наибольших элементов в порядке убывания. Используйте `slices.SortFunc`.
2. Создайте тип `Employee` с полями `Name string` и `Salary int`. Напишите функцию, которая сортирует слайс сотрудников по зарплате (убывание), а при равной зарплате — по имени (возрастание).
3. Напишите функцию `RemoveDuplicates(s []string) []string`, которая удаляет дубликаты из слайса строк. Подсказка: отсортируйте, затем используйте `slices.Compact`.
