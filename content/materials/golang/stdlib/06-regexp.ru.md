...
1. Напишите функцию `ExtractEmails(text string) []string`, которая находит все email-адреса в тексте с помощью регулярного выражения.
2. Напишите функцию `ValidatePassword(pass string) error`, которая проверяет пароль: минимум 8 символов, хотя бы одна цифра, хотя бы одна заглавная буква. Используйте `regexp.MatchString`.
3. Напишите функцию `MaskPhoneNumbers(text string) string`, которая заменяет телефонные номера вида `+7-999-123-45-67` на `+7-***-***-**-67` (скрывает всё кроме последних двух цифр). Используйте `ReplaceAllStringFunc`.
