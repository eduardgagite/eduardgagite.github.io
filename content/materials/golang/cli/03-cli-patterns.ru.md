...
1. Перепишите любую свою утилиту, используя паттерн `func run(ctx context.Context) error` в main. Убедитесь, что ошибки выводятся в stderr, а код завершения корректный.
2. Добавьте graceful shutdown: перехватите SIGINT/SIGTERM через `signal.NotifyContext` и дождитесь завершения текущей операции перед выходом.
3. Напишите утилиту, которая читает конфиг из трёх источников (флаг, переменная окружения, YAML-файл) с приоритетом: флаги > env > файл > значение по умолчанию.
