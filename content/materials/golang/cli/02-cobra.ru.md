...
1. Создайте CLI-приложение `taskman` с Cobra, которое имеет подкоманды `add <task>`, `list` и `done <id>`. Храните задачи в JSON-файле.
2. Добавьте глобальный флаг `--verbose` (PersistentFlag на rootCmd), который включает подробный вывод во всех подкомандах. Используйте `PersistentPreRunE` для инициализации логгера.
3. Добавьте автодополнение значений для флага `--priority` с вариантами `low`, `medium`, `high` через `RegisterFlagCompletionFunc`.
