...
1. Напишите утилиту `greet`, которая принимает флаги `--name` (строка, обязательный) и `--count` (число, по умолчанию 1) и выводит приветствие указанное количество раз.
2. Добавьте к утилите подкоманды `hello` и `bye` через `flag.NewFlagSet`, каждая со своим набором флагов.
3. Напишите утилиту `timer`, которая принимает флаг `--duration` (например, `"5s"`, `"1m30s"`) и ведёт обратный отсчёт до нуля с выводом оставшегося времени.

## Смотрите также

- **Cobra** — продвинутый фреймворк для CLI с подкомандами, автодополнением и --long-flags (раздел «CLI-разработка → Cobra»).
- **Конфигурация и параметры запуска** — комбинирование флагов с переменными окружения и конфиг-файлами (раздел «I/O и данные → Конфигурация и параметры запуска»).
