...
1. Создайте HTTP-сервер с эндпоинтами: `GET /health` (возвращает "ok"), `GET /time` (возвращает текущее время в JSON).
2. Добавьте эндпоинт `POST /echo`, который принимает JSON-тело и возвращает его обратно.
3. Реализуйте Graceful Shutdown для вашего сервера и проверьте, что он корректно завершает обработку текущих запросов.

## Смотрите также

- **Маршрутизация и Middleware** — как структурировать обработчики, цепочки middleware и обработка ошибок (раздел «Веб и API → Маршрутизация и Middleware»).
- **HTTP-клиент** — как делать запросы к внешним сервисам (раздел «Веб и API → HTTP-клиент»).
- **Context и отмена** — как использовать контекст для таймаутов и отмены запросов (раздел «Конкурентность → Context и отмена»).
