...
1. Создайте программу с переменными `Version`, `BuildTime`, `GitCommit`. Соберите её с `ldflags`, подставив реальные значения. Добавьте подкоманду `version`, которая выводит эту информацию.
2. Напишите Makefile с целями `build`, `test`, `run`, `clean`. Цель `build` должна встраивать версию из git-тега.
3. Сравните размер бинарника с флагами `-ldflags "-s -w"` и без них. Оцените, сколько процентов экономит удаление отладочной информации.

## Смотрите также

- **Кросс-компиляция и релизы** — сборка для разных платформ и Docker (раздел «Сборка и деплой → Кросс-компиляция и релизы»).
- **CI/CD и GitHub Actions** — автоматизация тестов и сборки (раздел «Сборка и деплой → CI/CD и GitHub Actions»).
