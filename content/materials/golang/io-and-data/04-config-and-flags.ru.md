...
1. Напишите программу, которая принимает флаги `-host`, `-port`, `-debug` и выводит конфигурацию. Если флаг не задан — берите значение из переменной окружения.
2. Создайте структуру `Config` и функцию `LoadConfig(path string)`, которая читает конфигурацию из JSON-файла. Обработайте случай, когда файл не существует (используйте значения по умолчанию).

## Смотрите также

- **Пакет flag подробнее** — продвинутое использование флагов, подкоманды и Cobra (раздел «CLI-разработка → Пакет flag» и «CLI-разработка → Cobra»).
- **JSON и кодирование** — подробнее о чтении и записи JSON (раздел «I/O и данные → JSON и кодирование»).
