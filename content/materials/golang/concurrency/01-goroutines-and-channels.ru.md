...
1. Запустите 5 горутин, каждая из которых выводит свой номер. Используйте `sync.WaitGroup` для ожидания завершения всех горутин.
2. Напишите программу «пинг-понг»: две горутины перебрасывают число через канал, увеличивая его на 1 каждый раз. Остановитесь, когда число достигнет 10.
3. Реализуйте функцию `merge(ch1, ch2 <-chan int) <-chan int`, которая объединяет два канала в один.

## Смотрите также

- **select и таймауты** — как ожидать несколько каналов одновременно и обрабатывать таймауты (раздел «Конкурентность → select и таймауты»).
- **Паттерны конкурентности** — worker pool, fan-out/fan-in, pipeline, errgroup (раздел «Конкурентность → Паттерны»).
- **Context и отмена** — как правильно останавливать горутины (раздел «Конкурентность → Context и отмена»).
