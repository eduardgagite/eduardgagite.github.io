...
1. Напишите функцию `divide(a, b float64) (float64, error)`, которая возвращает ошибку при делении на ноль.
2. Напишите функцию `parseAge(s string) (int, error)`, которая принимает строку, преобразует в число и возвращает ошибку, если возраст отрицательный или больше 150.
3. Напишите программу, которая открывает файл, читает его содержимое и корректно обрабатывает все ошибки (файл не найден, ошибка чтения).

## Смотрите также

- **Оборачивание ошибок** — как добавлять контекст к ошибкам через `%w`, использовать `errors.Is` и `errors.As` (раздел «Лучшие практики → Оборачивание ошибок»).
- **defer, panic и recover** — подробнее о panic и когда её допустимо использовать (раздел «Базовые конструкции → defer, panic и recover»).
