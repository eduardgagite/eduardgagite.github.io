...
1. Напишите функцию `StructToMap(v any) map[string]any`, которая конвертирует любую структуру в map, используя имена полей как ключи. Экспортированные поля только.
2. Напишите функцию `PrintFields(v any)`, которая выводит все поля структуры с их типами и значениями, учитывая теги `json` для имён.
3. Напишите функцию `SetDefaults(v any)`, которая заполняет нулевые поля структуры значениями из тега `default` (например, `default:"hello"` для строк, `default:"42"` для int).
