...
1. Создайте файл `version.txt` с текстом версии и встройте его в программу через `//go:embed`. Выведите версию при запуске с флагом `--version`.
2. Встройте директорию `templates/` с HTML-шаблонами через `embed.FS`. Напишите HTTP-сервер, который рендерит шаблоны из встроенной FS.
3. Встройте SQL-миграции из `migrations/*.sql` и напишите функцию, которая применяет их к базе данных в алфавитном порядке, используя `embed.FS.ReadDir`.
